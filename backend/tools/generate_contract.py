...
def write(path,content):
    if path.suffix == '.go':
        content = subprocess.run(['gofmt'],input=content,text=True,capture_output=True,check=True).stdout
...
for package,path,tags in (("domain","internal/domain/types_generated.go",False),("dto","internal/transport/http/dto/types_generated.go",True),("agenthttp","internal/adapters/agenthttp/types_generated.go",True)):
    ...
    for name,schema in S.items():
        ...
        code+='}\n\n'
    p=ROOT/path;p.parent.mkdir(parents=True,exist_ok=True);write(p,code)
    if tags:
        code=f'// Code generated by tools/generate_contract.py; DO NOT EDIT.\npackage {package}\nimport ("time";"wind/backend/internal/domain")\n'
            ...
            for direction in ("FromDomain","ToDomain"):
                ...
                for key,s in schema["properties"].items():
                    ...
                    if "$ref" in s:code+=f' out.{f} = {gotype(s)}{direction}(v.{f})\n'
                        ...
                        code+=f'out.{f}[i] = {typ}{direction}(item)' if "$ref" in item else f'out.{f}[i] = item'+('.UTC()' if item.get('format')=='date-time' else '')
                        code+=' }\n'
                    elif "anyOf" in s:code+=f' if v.{f} != nil {{ item := '+(f'v.{f}.UTC()' if s['anyOf'][0].get('format')=='date-time' else f'*v.{f}')+f'; out.{f} = &item }}\n'
                    else:code+=f' out.{f} = v.{f}'+('.UTC()' if s.get('format')=='date-time' else '')+'\n'
                code+=' return out\n}\n'
        write(p.parent/"mapping_generated.go",code)
...
for name,schema in S.items():
    ...
    typescript+='}\n'
p=ROOT/"contracts/typescript/api-types.ts";p.parent.mkdir(parents=True,exist_ok=True);write(p,typescript)
