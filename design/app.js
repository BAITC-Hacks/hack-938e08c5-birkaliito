...
import {api,historyRun,attachResult,waitForForecast,downloadBlob} from './api.mjs';
...
let run=makePreview({key:'new-draft',stage:'initial'});
const history=[];
const state={page:'forecast',reportRunId:run.key,reportTab:'hourly',quantileDetails:false,largeText:false,draft:{horizon:48},chartTurbine:'both',tablePage:0,filters:{from:'',to:'',turbine:'all',status:'all',mode:'all',sort:'desc'},energy:null,energyDirty:false,energyInput:{p1:'',p2:'',plan:'',confirmed:false},dataQuality:null};
...
function config(){
 ...
 <div class="forecast-intro"><h2>Прогноз для двух турбин</h2><p>Выберите горизонт — остальное рассчитает backend.</p></div>
 <div class="field"><label>На сколько вперёд</label><div class="segmented" role="group" aria-label="Горизонт прогноза">${[24,48].map(h=>`<button type="button" data-horizon="${h}" aria-pressed="${state.draft.horizon===h}" class="${state.draft.horizon===h?'active':''}">${h} часа</button>`).join('')}</div></div>
 <button id="forecast-submit" class="button primary" type="submit" ${!backend.available||['queued','running','cancel_requested','submitting'].includes(run.stage)?'disabled':''}>${icon('play')}<span>Рассчитать</span></button></form>
 ${!backend.available?'<p class="config-status" role="status">Подключаем backend…</p>':''}</section>`;
}
function chart(){
...
function turbineForecast(id){
 const rows=run.points.filter(point=>point.turbine_id===id),W=520,H=190,left=22,right=508,top=12,bottom=155;
 if(!rows.length)return '';
 const x=index=>left+index/Math.max(rows.length-1,1)*(right-left),y=value=>bottom-Math.max(0,Math.min(1,value))*(bottom-top);
 const line=key=>rows.map((point,index)=>`${index?'L':'M'}${x(index).toFixed(1)},${y(point[key]).toFixed(1)}`).join(' ');
 const band=`${line('q90')} ${[...rows].reverse().map((point,index)=>`L${x(rows.length-index-1).toFixed(1)},${y(point.q10).toFixed(1)}`).join(' ')} Z`;
 const average=rows.reduce((sum,point)=>sum+point.power_mean,0)/rows.length,color=id==='1'?'#24634E':'#426889';
 return `<article class="card turbine-forecast"><div class="card-heading"><div><span class="eyebrow">ВЭУ 0${id}</span><h2>Турбина ${id}</h2></div><strong>${num(average,2)}</strong></div><p class="muted small">Средняя нормализованная мощность · ${run.horizon} ч</p><svg class="forecast-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="Прогноз мощности турбины ${id}">${[0,.5,1].map(value=>`<line x1="${left}" x2="${right}" y1="${y(value)}" y2="${y(value)}" stroke="#DDE5DF" stroke-width="1"/><text x="0" y="${y(value)+4}" fill="#64756C" font-size="12">${num(value,value===.5?1:0)}</text>`).join('')}<path d="${band}" fill="${color}" opacity=".10"/><path d="${line('power_mean')}" fill="none" stroke="${color}" stroke-width="3"/><text x="${left}" y="182" fill="#64756C" font-size="12">${csvTime(rows[0].valid_time).slice(5,16)}</text><text x="${right}" y="182" text-anchor="end" fill="#64756C" font-size="12">${csvTime(rows.at(-1).valid_time).slice(5,16)}</text></svg></article>`;
}
const turbineForecasts=()=>`<section class="equal-grid turbine-grid">${run.turbines.map(turbineForecast).join('')}</section>`;
function forecastPage(){
 const pending=!hasResult(run)&&run.stage!=='initial';
 return `${config()}${pending?`<div class="section-gap">${currentNotice()}</div>`:''}${hasResult(run)?`<div class="result-heading"><div><h2>Прогноз на ${run.horizon} часов</h2><p>${compactTime(run.origin)} · обе турбины</p></div><div class="heading-actions">${button('Скачать CSV','export','ghost','download')}${button('История','show-analysis','primary','clock')}</div></div>${turbineForecasts()}`:`<section class="card section-gap">${empty('Прогноз появится здесь','Выберите 24 или 48 часов и нажмите «Рассчитать».','wind')}</section>`}`;
}
...
const pages={home:homePage,forecast:forecastPage,reports:reportsPage,data:dataPage};
function render(){
 ...
 const pageTitles={forecast:'Прогноз',reports:'Аналитика',data:'Исходные данные'};
...
async function loadRun(next,destination='forecast'){
 ...
 if(destination==='forecast')state.draft={horizon:run.horizon};
...
function passport(){
 ...
 ])}<p class="notice-text">${run.mode==='fixture'?'Синтетический режим разработки: значения не являются реальным прогнозом.':'Поля получены из результата backend.'}</p><div class="modal-actions">${button(run.remote?'CSV backend':'CSV иллюстрации','export','primary','download',!hasResult(run)?'disabled':'')}</div>`);
}
function bindChart(){
...
async function startPreview(){
 ...
 const turbines=['1','2'],origin=new Date();origin.setUTCMinutes(0,0,0);
 const originISO=origin.toISOString();
 const next=makePreview({key:'pending',origin:originISO,horizon:state.draft.horizon,turbines,stage:'submitting'});
 next.points=[];next.remote=true;next.model=backend.modelVersion;next.mode='fixture';run=next;render();
 try{
  const job=await api.createForecast({forecast_origin:originISO,horizon_hours:state.draft.horizon,turbine_ids:[1,2],mode:'live',model_version:backend.modelVersion,data_mode:'fixture'},crypto.randomUUID());
...
async function connectBackend(){
 try{
  ...
  const quality=await Promise.allSettled([backend.capabilities.data_quality?api.dataQuality():Promise.resolve(null)]);
  state.dataQuality=quality[0].status==='fulfilled'?quality[0].value:null;
...
document.addEventListener('submit',e=>{
 ...
 if(form.id==='forecast-form')void startPreview();
...
document.addEventListener('click',e=>{
 ...
 const target=e.target.closest('button,[data-open-run]');if(!target||target.disabled)return;
 ...
 if(action==='new-preview'){closeModal();run=makePreview({key:'new-draft',stage:'initial',horizon:state.draft.horizon,turbines:['1','2']});state.page='forecast';state.energy=null;state.tablePage=0;location.hash='forecast';render();$('#forecast-submit')?.focus();}
 ...
 if(action==='help')modal('Как пользоваться AURA','<p>Выберите прогноз на 24 или 48 часов и нажмите «Рассчитать». Backend рассчитает обе турбины, а интерфейс покажет их отдельно.</p>');
 if(action==='prototype-info')modal('Статус реализации',`<p>Прогноз для обеих турбин рассчитывает Go backend. Доступны только горизонты 24 и 48 часов.</p><p>${esc(backend.message)}</p><p>Режим fixture содержит синтетические данные.</p>`);
});
