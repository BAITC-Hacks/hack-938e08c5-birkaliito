...
Для графика используйте `turbine_id`, `valid_time`, `power_mean`, `q10`, `q50`, `q90`. Среднее и медиана могут отличаться. Для replay пересекающиеся `valid_time` относятся к разным моментам выпуска и не должны сливаться без отдельного правила. CSV прогнозов и replay получает клиент через `exportForecast` и `exportReplay`; для неполного replay требуется `allowPartial=true`, а заголовок экспорта помечает частичный результат.
